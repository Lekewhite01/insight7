...
def get_answer(docs: List[Document], query: str) -> Dict[str, Any]:
    ...
    chain = load_qa_with_sources_chain(
        OpenAI(
            temperature=0.7,model_name="text-davinci-003",max_tokens=1000),  # type: ignore,# openai_api_key=st.session_state.get("OPENAI_API_KEY")
        chain_type="stuff",
        prompt=STUFF_PROMPT,
    )
...
def extract_topics(document):
    ...
    for match in matches:
        ...
        pain_points = match[2].split(', ')
        desires = match[3].split(', ')
        behaviour = match[4].split(', ')
        ...
        topics_dict[topic_name] = topic_dict
...
def parse_topic(text):
    ...
    topics = re.split(r"\nTopic \d+:", text)
    topics[0] = topics[0].replace('Topic 1:', ' ')
    ...
    for topic in topics:
        ...
        for i, group in enumerate(groups):
            if i == 0:
                topic = group.split('\n')[0]
                topic = re.sub(r'^[\W_0-9]+', '', topic).strip()
...
def get_result():
    ...
    while wrong_format:
        try:
            ...
            txt = answer['output_text']
            response = extract_insights(txt)
            result = parse_topic(response) 
            return result
        except:
            pass
...
def index():
    try:
        ...
        if (content_type == 'text/plain'):
            ...
            with open((os.path.join(directory_name, "text.txt")), "w") as file:
                file.write(str(data))
            doc = open('./data/text.txt','r')
            # print(doc)
            text = text_to_docs(doc)
            index = embed_docs(text)
            sources = search_docs(index, query)
...
# output = prepare_json_format(response)
